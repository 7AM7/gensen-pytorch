...
class ClassificationIterator:
    """Data iterator for classification datasets (e.g.: Dialects_Detection)"""
    ...
    def __init__(
        ...
        """
        Prepare minibatch for classification
        ...
        logging.info("Reading classification training data")
        for taskname in self.src:
            self.fetch_buffer(taskname)

        logging.info("Reading and tokenizing classification validation data")
    ...
    def get_parallel_minibatch(self, taskname, index, batch_size, max_len_src, minibatch_type="train"):
        """
        Prepare minibatch for classification tasks
        ...
        :return: (dict) a ِminibatch of sentences and their labels
        """
        ...
        sent_lens = [len(line) for line in sent]
        ...
        sent = [
            ...
            + [self.tokenizer.token_to_id("<pad>")] * (max_sent_len - len(line))
            for line in sent
        ]

        sent = torch.tensor(sent).to(self.device)
        labels = torch.tensor(labels).to(self.device)
        sent_lens = torch.tensor(sent_lens, requires_grad=False).squeeze().to(self.device)
